...
namespace SIFT_SLAM3
{
    ...
    int SIFTmatcher::SearchByProjection(Frame &F, const vector<MapPoint*> &vpMapPoints, const float th, const bool bFarPoints, const float thFarPoints)
    {
    ...
    int SIFTmatcher::SearchByBoW(KeyFrame* pKF,Frame &F, vector<MapPoint*> &vpMapPointMatches)
    {
        ...
        // We perform the matching over SIFT that belong to the same vocabulary node (at a certain level)
        ...
        while(KFit != KFend && Fit != Fend)
        {
            if(KFit->first == Fit->first)
            {
                ...
                for(size_t iKF=0; iKF<vIndicesKF.size(); iKF++)
                {
                    ...
                    float bestDist1R=2.0;
                    int bestIdxFR =-1 ;
                    float bestDist2R=2.0;
    ...
    int SIFTmatcher::SearchByProjection(KeyFrame* pKF, Sophus::Sim3f &Scw, const vector<MapPoint*> &vpPoints,
    ...
    int SIFTmatcher::SearchByProjection(KeyFrame* pKF, Sophus::Sim3<float> &Scw, const std::vector<MapPoint*> &vpPoints, const std::vector<KeyFrame*> &vpPointsKFs,
    ...
    int SIFTmatcher::SearchForInitialization(Frame &F1, Frame &F2, vector<cv::Point2f> &vbPrevMatched, vector<int> &vnMatches12, int windowSize)
    {
        ...
        vector<float> vMatchedDistance(F2.mvKeysUn.size(),2.0);
        ...
        for(size_t i1=0, iend1=F1.mvKeysUn.size(); i1<iend1; i1++)
        {
            ...
            if(bestDist<=TH_LOW)
            {
                if(bestDist<bestDist2*mfNNratio)
                {
    ...
    int SIFTmatcher::SearchForTriangulation(KeyFrame *pKF1, KeyFrame *pKF2,
                                           vector<pair<size_t, size_t> > &vMatchedPairs, const bool bOnlyStereo, const bool bCoarse)
    {
        ...
        // Matching speed-up by SIFT Vocabulary
        // Compare only SIFT that share the same node
        int nmatches=0;
                ...
                for(size_t i1=0, iend1=f1it->second.size(); i1<iend1; i1++)
                {
                    ...
                    for(size_t i2=0, iend2=f2it->second.size(); i2<iend2; i2++)
                    {
                        ...
                        if(!bStereo1 && !bStereo2 && !pKF1->mpCamera2)
                        {
                            ...
                            // ToDo: Sqrt(2) factor comes from CudaSift example. Not sure it's meaningful
                            if(distex*distex+distey*distey<100*1.41*kp2.size)
                            {
                                continue;
                            }
                        }
    ...
    int SIFTmatcher::Fuse(KeyFrame *pKF, const vector<MapPoint *> &vpMapPoints, const float th, const bool bRight)
    {
    ...
    int SIFTmatcher::Fuse(KeyFrame *pKF, Sophus::Sim3f &Scw, const vector<MapPoint *> &vpPoints, float th, vector<MapPoint *> &vpReplacePoint)
    {
        // Get Calibration Parameters for later projection
        const float &fx = pKF->fx;
        const float &fy = pKF->fy;
    ...
    int SIFTmatcher::SearchBySim3(KeyFrame* pKF1, KeyFrame* pKF2, std::vector<MapPoint *> &vpMatches12, const Sophus::Sim3f &S12, const float th)
    {
    ...
    int SIFTmatcher::SearchByProjection(Frame &CurrentFrame, KeyFrame *pKF, const set<MapPoint*> &sAlreadyFound, const float th , const float SIFTdist)
    {
        int nmatches = 0;
    ...
    int SIFTmatcher::SearchForInitializationFromOdometery(Frame &F1, Frame &F2, vector<cv::Point2f> &vbPrevMatched, vector<int> &vnMatches12)
    {
        int nmatches=F2.matches.size();
        vnMatches12 = vector<int>(F1.mvKeysUn.size(),-1);

        for(int i=0; i<F2.matches.size(); i++)
            vnMatches12[F2.matches[i].queryIdx]=F2.matches[i].trainIdx;
    ...
    float SIFTmatcher::DescriptorDistance(const cv::Mat &a, const cv::Mat &b)
    {
        float dist = cv::norm(a, b); // default is l2 norm      
        return dist;
    }
