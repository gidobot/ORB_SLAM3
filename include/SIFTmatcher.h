...
namespace SIFT_SLAM3
{
    ...
    public:
        ...
        // Computes the L2 distance between two SIFT descriptors
        ...
        int SearchByProjection(Frame &CurrentFrame, KeyFrame* pKF, const std::set<MapPoint*> &sAlreadyFound, const float th, const float SIFTdist);

        // Project MapPoints using a Similarity Transformation and search matches.
        // Used in loop detection (Loop Closing)
        int SearchByProjection(KeyFrame* pKF, Sophus::Sim3<float> &Scw, const std::vector<MapPoint*> &vpPoints, std::vector<MapPoint*> &vpMatched, int th);

        // Project MapPoints using a Similarity Transformation and search matches.
        // Used in Place Recognition (Loop Closing and Merging)
        int SearchByProjection(KeyFrame* pKF, Sophus::Sim3<float> &Scw, const std::vector<MapPoint*> &vpPoints, const std::vector<KeyFrame*> &vpPointsKFs, std::vector<MapPoint*> &vpMatched, std::vector<KeyFrame*> &vpMatchedKF, int th);

        // Search matches between MapPoints in a KeyFrame and SIFT in a Frame.
        // Brute force constrained to SIFT that belong to the same vocabulary node (at a certain level)
        ...
        // Matching from viso2 odometery for the Map Initialization (only used in the monocular case)
        int SearchForInitializationFromOdometery(Frame &F1, Frame &F2, vector<cv::Point2f> &vbPrevMatched, std::vector<int> &vnMatches12);
