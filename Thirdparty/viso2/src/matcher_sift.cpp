...
Matcher_SIFT::Matcher_SIFT(parameters param) : param(param) {
...
Matcher_SIFT::~Matcher_SIFT() {
  ...
  FreeSiftData(siftdata_c_1);
  FreeSiftData(siftdata_p_1);
  FreeSiftData(siftdata_c_2);
  FreeSiftData(siftdata_p_2);
}

void Matcher_SIFT::copyMatchData(std::vector<cv::KeyPoint> &kpts_l, std::vector<cv::KeyPoint> &kpts_r, cv::Mat &desc_l, cv::Mat &desc_r,
  std::vector<Matcher_SIFT::p_match> &matches_circle, std::vector<cv::DMatch> &matches_lr) {
  kpts_l = keypoints_c_1;
  kpts_r = keypoints_c_2;
  desc_l = descriptors_c_1.clone();
  desc_r = descriptors_c_2.clone();
  matches_circle = p_matched_2;
  matches_lr = cf_matches;
}

void Matcher_SIFT::copyMatchData(std::vector<cv::KeyPoint> &kpts, cv::Mat &desc,
  std::vector<Matcher_SIFT::p_match> &matches_viso, std::vector<cv::DMatch> &matches_cv) {
  kpts = keypoints_c_1;
  desc = descriptors_c_1.clone();
  matches_viso = p_matched_2;
...
void Matcher_SIFT::pushBackCUDA(const cv::Mat& I1, const cv::Mat& I2, const bool replace) {
  ...
  if (replace) {
    ...
    image_p_2 = image_c_2.clone();
    siftdata_p_1 = siftdata_c_1;
    siftdata_p_2 = siftdata_c_2;
  }
...
void Matcher_SIFT::pushBack(const cv::Mat& I1, const cv::Mat& I2, const bool replace) {
  ...
  computeFeatures(I1, keypoints_c_1, descriptors_c_1);
  if (!I2.empty())
    computeFeatures(I2, keypoints_c_2, descriptors_c_2);
  // std::cout << "Num keypoints : " << keypoints_c_1.size() << ", " << keypoints_c_2.size() << std::endl; 
}

void Matcher_SIFT::pushBack (uint8_t *I1,uint8_t* I2,int32_t* dims,const bool replace) {
...
void Matcher_SIFT::matchFeaturesCUDA(int32_t method, const bool reset, Matrix *Tr_delta) {
  ...
  // flow
  if (method==0) {
    if (siftdata_c_1.numPts == 0 || siftdata_p_1.numPts == 0)
      return;
  // stereo
  } else {
    if (siftdata_c_1.numPts == 0 || siftdata_c_2.numPts == 0 || siftdata_p_1.numPts == 0 || siftdata_p_2.numPts == 0)
      return;
  }
...
void Matcher_SIFT::matchFeaturesSIFT(int32_t method, const bool reset, Matrix *Tr_delta) {
  
  //////////////////
  // sanity check //
  //////////////////

  if (keypoints_c_1.empty() || keypoints_c_2.empty() || keypoints_p_1.empty() || keypoints_p_2.empty())
    return;
  ...
  // single pass matching
    ...
    if (reset)
      matchingBF(keypoints_p_1,keypoints_p_2,keypoints_c_1,keypoints_c_2,descriptors_p_1,descriptors_p_2,descriptors_c_1,descriptors_c_2,p_matched_2,method,false,Tr_delta);
    else
      matching(keypoints_p_1,keypoints_p_2,keypoints_c_1,keypoints_c_2,descriptors_p_1,descriptors_p_2,descriptors_c_1,descriptors_c_2,p_matched_2,method,false,Tr_delta);
...
void Matcher_SIFT::matchFeatures(int32_t method, Matrix *Tr_delta) {
...
void Matcher_SIFT::bucketFeatures(int32_t max_features,float bucket_width,float bucket_height) {
...
float Matcher_SIFT::getGain (vector<int32_t> inliers) {
...
void Matcher_SIFT::plotMatches(std::vector<int32_t> matches) {
  // draw matches
  std::vector<cv::KeyPoint> kpts_p_1;
  std::vector<cv::KeyPoint> kpts_p_2;
  std::vector<cv::KeyPoint> kpts_c_1;
  std::vector<cv::KeyPoint> kpts_c_2;
  std::vector<cv::DMatch> cvmatches;

  for (std::vector<Matcher_SIFT::p_match>::const_iterator it = p_matched_2.begin(); it!=p_matched_2.end(); it++) {
    kpts_p_1.push_back(cv::KeyPoint(it->u1p, it->v1p, 1));
    kpts_p_2.push_back(cv::KeyPoint(it->u2p, it->v2p, 1));
    kpts_c_1.push_back(cv::KeyPoint(it->u1c, it->v1c, 1));
    kpts_c_2.push_back(cv::KeyPoint(it->u2c, it->v2c, 1));
  }

  for (std::vector<int32_t>::const_iterator it=matches.begin(); it!=matches.end(); it++) {
    if (*it<(int32_t)p_matched_2.size())
      cvmatches.push_back(cv::DMatch(*it, *it, 0));
  }

  // cvmatches.resize(matches.size());
  // for (vector<int32_t>::iterator it=matches.begin(); it!=matches.end(); it++) {
  //   if (*it<(int32_t)p_matched_2.size())
  //     cvmatches.push_back(cv::DMatch(p_matched_2[*it].i1c, p_matched_2[*it].i1p, 0));
      // cvmatches.push_back(cv::DMatch(p_matched_2[*it].i1c, p_matched_2[*it].i2c, 0));
      // cvmatches.push_back(cv::DMatch(p_matched_2[*it].i1p, p_matched_2[*it].i2p, 0));
  // }
...
void Matcher_SIFT::nonMaximumSuppression (int16_t* I_f1,int16_t* I_f2,const int32_t* dims,vector<Matcher_SIFT::maximum> &maxima,int32_t nms_n) {
  ...
  for (int32_t i=n+margin; i<width-n-margin;i+=n+1) {
    for (int32_t j=n+margin; j<height-n-margin;j+=n+1) {
      ...
      if (f1minval<=-tau)
        maxima.push_back(Matcher_SIFT::maximum(f1mini,f1minj,f1minval,0));
      failed_f1min:;
      ...
      if (f1maxval>=tau)
        maxima.push_back(Matcher_SIFT::maximum(f1maxi,f1maxj,f1maxval,1));
      failed_f1max:;
      ...
      if (f2minval<=-tau)
        maxima.push_back(Matcher_SIFT::maximum(f2mini,f2minj,f2minval,2));
      failed_f2min:;
      ...
      if (f2maxval>=tau)
        maxima.push_back(Matcher_SIFT::maximum(f2maxi,f2maxj,f2maxval,3));
      failed_f2max:;
    }
  }
}

inline void Matcher_SIFT::computeDescriptor (const uint8_t* I_du,const uint8_t* I_dv,const int32_t &bpl,const int32_t &u,const int32_t &v,uint8_t *desc_addr) {
...
inline void Matcher_SIFT::computeSmallDescriptor (const uint8_t* I_du,const uint8_t* I_dv,const int32_t &bpl,const int32_t &u,const int32_t &v,uint8_t *desc_addr) {
...
void Matcher_SIFT::computeDescriptors (uint8_t* I_du,uint8_t* I_dv,const int32_t bpl,std::vector<Matcher_SIFT::maximum> &maxima) {
  ...
  // for all maxima do
  for (vector<Matcher_SIFT::maximum>::iterator it=maxima.begin(); it!=maxima.end(); it++) {
    u = (*it).u;
    v = (*it).v;
...
inline uint8_t Matcher_SIFT::saturate (int16_t in) {
  if (in<0)   return 0;
  if (in>255) return 255;
  return in;
}

void Matcher_SIFT::filterImageAll (uint8_t* I,uint8_t* I_du,uint8_t* I_dv,int16_t* I_f1,int16_t* I_f2,const int* dims) {
...
void Matcher_SIFT::filterImageSobel (uint8_t* I,uint8_t* I_du,uint8_t* I_dv,const int* dims) {
...
void Matcher_SIFT::getHalfResolutionDimensions(const int32_t *dims,int32_t *dims_half) {
  dims_half[0] = dims[0]/2;
  dims_half[1] = dims[1]/2;
  dims_half[2] = dims_half[0]+15-(dims_half[0]-1)%16;
}

uint8_t* Matcher_SIFT::createHalfResolutionImage(uint8_t *I,const int32_t* dims) {
  int32_t dims_half[3];
...
void Matcher_SIFT::copyCUDAFeatures() {
  ...
  #else
    SiftPoint *sift1c = siftdata_c_1.h_data;
    SiftPoint *sift2c = siftdata_c_2.h_data;
  #endif
...
void Matcher_SIFT::computeFeatures(const cv::Mat& I, std::vector<cv::KeyPoint> &keypoints, cv::Mat &descriptors) {
  sift_feature->detectAndCompute(I, cv::noArray(), keypoints, descriptors);
...
void Matcher_SIFT::computeFeatures(const cv::Mat& I, SiftData &siftdata) {
#ifdef PROFILING
  std::chrono::steady_clock::time_point t1 = std::chrono::steady_clock::now();
#endif
  cv::Mat gI;
  I.convertTo(gI, CV_32FC1);
  float scale = std::min(float(param.max_dim)/float(gI.cols), float(param.max_dim)/float(gI.rows));
  if (scale > 1.0) scale = 1.0;
  cv::resize(gI, gI, cv::Size(), scale, scale, cv::INTER_LINEAR);
  CudaImage img;
  ...
  float *memoryTmpCUDA = AllocSiftTempMemory(I.cols, I.rows, 5, false);
  ExtractSift(siftdata, img, 5, initBlur, thresh, 0.0f, false, memoryTmpCUDA, 1.0f/scale);
  FreeSiftTempMemory(memoryTmpCUDA);
#ifdef PROFILING
  std::chrono::steady_clock::time_point t4 = std::chrono::steady_clock::now();
  cout << "CudaSift Profiling (Init Image Data): " << std::chrono::duration_cast<std::chrono::duration<double> >(t2 - t1).count() << endl;
  cout << "CudaSift Profiling (Download): " << std::chrono::duration_cast<std::chrono::duration<double> >(t3 - t2).count() << endl;
  cout << "CudaSift Profiling (Extract): " << std::chrono::duration_cast<std::chrono::duration<double> >(t4 - t3).count() << endl;
#endif
}

void Matcher_SIFT::computeFeatures (uint8_t *I,const int32_t* dims,int32_t* &max1,int32_t &num1,int32_t* &max2,int32_t &num2,uint8_t* &I_du,uint8_t* &I_dv,uint8_t* &I_du_full,uint8_t* &I_dv_full) {
  ...
  vector<Matcher_SIFT::maximum> maxima1;
  if (param.multi_stage) {
  ...
  vector<Matcher_SIFT::maximum> maxima2;
  ...
  if (num1!=0) {
    max1 = (int32_t*)_mm_malloc(sizeof(Matcher_SIFT::maximum)*num1,16);
    int32_t k=0;
    for (vector<Matcher_SIFT::maximum>::iterator it=maxima1.begin(); it!=maxima1.end(); it++) {
  ...
  if (num2!=0) {
    max2 = (int32_t*)_mm_malloc(sizeof(Matcher_SIFT::maximum)*num2,16);
    int32_t k=0;
    for (vector<Matcher_SIFT::maximum>::iterator it=maxima2.begin(); it!=maxima2.end(); it++) {
...
void Matcher_SIFT::computePriorStatistics (vector<Matcher_SIFT::p_match> &p_matched,int32_t method) {
  ...
  vector<Matcher_SIFT::delta> *delta_accu = new vector<Matcher_SIFT::delta>[bin_num];
  
  // fill bin accumulator
  Matcher_SIFT::delta delta_curr;
  for (vector<Matcher_SIFT::p_match>::iterator it=p_matched.begin(); it!=p_matched.end(); it++) {
  ...
  for (int32_t v_bin=0; v_bin<v_bin_num; v_bin++) {
    for (int32_t u_bin=0; u_bin<u_bin_num; u_bin++) {
      ...
      if (delta_accu[v_bin*u_bin_num+u_bin].size()>0) {
        ...
        for (vector<Matcher_SIFT::delta>::iterator it=delta_accu[v_bin*u_bin_num+u_bin].begin();
...
void Matcher_SIFT::createIndexVector (std::vector<cv::KeyPoint> &keypoints, vector<int32_t> *k, const int32_t &u_bin_num, const int32_t &v_bin_num) {

  int32_t n = keypoints.size();

  // for all points do
  for (int32_t i=0; i<n; i++) {
    
    // extract coordinates and class
    float u = keypoints[i].pt.x; // u-coordinate
    float v = keypoints[i].pt.y; // v-coordinate
    
    // compute row and column of bin to which this observation belongs
    int32_t u_bin = min((int32_t)floor(u/(float)param.match_binsize),u_bin_num-1);
    int32_t v_bin = min((int32_t)floor(v/(float)param.match_binsize),v_bin_num-1);
...
void Matcher_SIFT::createIndexVector (int32_t* m,int32_t n,vector<int32_t> *k,const int32_t &u_bin_num,const int32_t &v_bin_num) {

  // descriptor step size
  int32_t step_size = sizeof(Matcher_SIFT::maximum)/sizeof(int32_t);
...
inline void Matcher_SIFT::findMatch (std::vector<cv::KeyPoint> &kpts1, cv::Mat &desc1, const int32_t &i1, std::vector<cv::KeyPoint> &kpts2, cv::Mat &desc2, vector<int32_t> *k2,
  ...
  float u1       = kpts1[i1].pt.x;
  float v1       = kpts1[i1].pt.y;
  ...
  for (int32_t u_bin=u_bin_min; u_bin<=u_bin_max; u_bin++) {
    for (int32_t v_bin=v_bin_min; v_bin<=v_bin_max; v_bin++) {
      int32_t k2_ind = v_bin*u_bin_num+u_bin;
      for (vector<int32_t>::const_iterator i2_it=k2[k2_ind].begin(); i2_it!=k2[k2_ind].end(); i2_it++) {
        float u2   = kpts2[*i2_it].pt.x;
        float v2   = kpts2[*i2_it].pt.y;
        if (u2>=u_min && u2<=u_max && v2>=v_min && v2<=v_max) {
          cv::Mat d1 = desc1.row(i1);
          cv::Mat d2 = desc2.row(*i2_it);
          double cost = 2 - 2*d1.dot(d2); // verify dot product correct
          // if (u_>=0 && v_>=0) {
...
inline void Matcher_SIFT::findMatch (int32_t* m1,const int32_t &i1,int32_t* m2,const int32_t &step_size,vector<int32_t> *k2,
                                const int32_t &u_bin_num,const int32_t &v_bin_num,const int32_t &stat_bin,
                                int32_t& min_ind,int32_t stage,bool flow,bool use_prior,double u_,double v_) {
  ...
  __m128i xmm1     = _mm_load_si128((__m128i*)(m1+step_size*i1+4)); // first half of descriptor
  __m128i xmm2     = _mm_load_si128((__m128i*)(m1+step_size*i1+8)); // second half of descriptor
    ...
    for (int32_t v_bin=v_bin_min; v_bin<=v_bin_max; v_bin++) {
      ...
      for (vector<int32_t>::const_iterator i2_it=k2[k2_ind].begin(); i2_it!=k2[k2_ind].end(); i2_it++) {
        ...
        if (u2>=u_min && u2<=u_max && v2>=v_min && v2<=v_max) {
          ...
          xmm3 = _mm_sad_epu8 (xmm1,xmm3); // sum absolute difference for first half descriptor
          xmm4 = _mm_sad_epu8 (xmm2,xmm4); // ... for second half descriptor
          xmm4 = _mm_add_epi16(xmm3,xmm4); // join results for two halves of descriptor
...
int32_t Matcher_SIFT::findNextMatchIDX(int32_t idx, std::vector< std::vector<cv::DMatch> > matches) {
  for (int32_t i=0; i<matches.size(); i++) {
    if (matches[i][0].queryIdx == idx)
      return i;
  }
  return -1;
}

void Matcher_SIFT::matchingBF(SiftData &data1c, SiftData &data2c, SiftData &data1p, SiftData &data2p,
                        vector<Matcher_SIFT::p_match> &p_matched, std::vector<cv::DMatch> &c_matches, int32_t method, bool use_prior, Matrix *Tr_delta) {
  // Since SIFT is a floating-point descriptor NORM_L2 is used
  // cv::Ptr<cv::DescriptorMatcher> matcher = cv::DescriptorMatcher::create(cv::DescriptorMatcher::BRUTEFORCE);
  int32_t u1p,v1p,u2p,v2p,u1c,v1c,u2c,v2c;
  int32_t i1p,i2p,i1c,i2c,i1p2,i1c2;
  ...
  if (method==0) {
    ...
    #else
      SiftPoint *sift1c = data1c.h_data;
      SiftPoint *sift1p = data1p.h_data;
    #endif

    for (int32_t i1p=0; i1p<data1p.numPts; i1p++) {
      if (sift1p[i1p].ambiguity > ratio_thresh)
        continue;
      i1c = sift1p[i1p].match;
      ...
      if (i1p == i1p2) {
        ...
        if (u1p>=u2p && u1c>=u2c) {
          // add match
          p_matched.push_back(Matcher_SIFT::p_match(u1p,v1p,i1p,-1,-1,-1,
                                               u1c,v1c,i1c,-1,-1,-1));
          c_matches.push_back(cv::DMatch(i1p, i1c, sift1p[i1p].score));
        }
      }
    }
  ...
  // method: quad matching
    ...
    for (int32_t i1c=0; i1c<data1c.numPts; i1c++) {
      ...
      if (i1c == i1c2) {
        // extract coordinates
        u2c = sift2c[i2c].xpos; v2c = sift2c[i2c].ypos;
        u1c = sift1c[i1c].xpos; v1c = sift1c[i1c].ypos;
    ...
    // MatchSiftData(data2c, data1c); // Already done above
    MatchSiftData(data1c, data1p);
    ...
    #else
      sift1c = data1c.h_data;
      sift2c = data2c.h_data;
      SiftPoint *sift1p = data1p.h_data;
      SiftPoint *sift2p = data2p.h_data;
    #endif
...
void Matcher_SIFT::matchingBF(vector<cv::KeyPoint> &kpts1p, vector<cv::KeyPoint> &kpts2p, vector<cv::KeyPoint> &kpts1c, vector<cv::KeyPoint> &kpts2c,
                        cv::Mat &desc1p, cv::Mat &desc2p, cv::Mat &desc1c, cv::Mat &desc2c,
                        vector<Matcher_SIFT::p_match> &p_matched, int32_t method, bool use_prior, Matrix *Tr_delta) {
  // Since SIFT is a floating-point descriptor NORM_L2 is used
  // cv::Ptr<cv::DescriptorMatcher> matcher = cv::DescriptorMatcher::create(cv::DescriptorMatcher::BRUTEFORCE);
  cv::Ptr<cv::DescriptorMatcher> matcher = cv::DescriptorMatcher::create(cv::DescriptorMatcher::FLANNBASED);
  std::vector< std::vector<cv::DMatch> > p1p2, p2c2, c2c1, c1p1;
  int32_t u1p,v1p,u2p,v2p,u1c,v1c,u2c,v2c;
  ...
  matcher->knnMatch(desc1p, desc2p, p1p2, 2);
  matcher->knnMatch(desc2p, desc2c, p2c2, 2);
  matcher->knnMatch(desc2c, desc1c, c2c1, 2);
  matcher->knnMatch(desc1c, desc1p, c1p1, 2);

  for (int32_t i1p=0; i1p<p1p2.size(); i1p++) {
    if (p1p2[i1p][0].distance > ratio_thresh * p1p2[i1p][1].distance)
      continue;
    i2p  = p1p2[i1p][0].trainIdx;
    if (fabs(kpts2p[i2p].pt.y - kpts1p[i1p].pt.y) > param.match_disp_tolerance)
      continue;

    if (p2c2[i2p][0].distance > ratio_thresh * p2c2[i2p][1].distance)
      continue;
    i2c  = p2c2[i2p][0].trainIdx;

    if (c2c1[i2c][0].distance > ratio_thresh * c2c1[i2c][1].distance)
      continue;
    i1c  = c2c1[i2c][0].trainIdx;
    if (fabs(kpts2c[i2c].pt.y - kpts1c[i1c].pt.y) > param.match_disp_tolerance)
      continue;

    if (c1p1[i1c][0].distance > ratio_thresh * c1p1[i1c][1].distance)
      continue;
    i1p2 = c1p1[i1c][0].trainIdx;

    if (i1p == i1p2) {
      // extract coordinates
      u2c = kpts2c[i2c].pt.x; v2c = kpts2c[i2c].pt.y;
      u1c = kpts1c[i1c].pt.x; v1c = kpts1c[i1c].pt.y;
      u2p = kpts2p[i2p].pt.x; v2p = kpts2p[i2p].pt.y;
      u1p = kpts1p[i1p].pt.x; v1p = kpts1p[i1p].pt.y;
...
void Matcher_SIFT::matching (vector<cv::KeyPoint> &kpts1p, vector<cv::KeyPoint> &kpts2p, vector<cv::KeyPoint> &kpts1c, vector<cv::KeyPoint> &kpts2c,
                        cv::Mat &desc1p, cv::Mat &desc2p, cv::Mat &desc1c, cv::Mat &desc2c,
                        vector<Matcher_SIFT::p_match> &p_matched, int32_t method, bool use_prior, Matrix *Tr_delta) {
  ...
  int32_t u_bin_num = (int32_t)ceil((float)dims_c[0]/(float)param.match_binsize);
  int32_t v_bin_num = (int32_t)ceil((float)dims_c[1]/(float)param.match_binsize);
  int32_t bin_num   = v_bin_num*u_bin_num; // 1 class for SIFT
  ...
  float u1p,v1p,u2p,v2p,u1c,v1c,u2c,v2c;
  ...
  if (method==0) {
    ...
    //       p_matched.push_back(Matcher_SIFT::p_match(u1p,v1p,i1p,-1,-1,-1,u1c,v1c,i1c,-1,-1,-1));
  ...
  // method: stereo
    ...
    //       // add match if this pixel isn't matched yet
    //       if (*(M+getAddressOffsetImage(u1c,v1c,dims_c[0]))==0) {
    //         p_matched.push_back(Matcher_SIFT::p_match(-1,-1,-1,-1,-1,-1,u1c,v1c,i1c,u2c,v2c,i2c));
    //         *(M+getAddressOffsetImage(u1c,v1c,dims_c[0])) = 1;
    //       }
    //     }
    //   }
  ...
  // method: quad matching
    ...
    // create position/class bin index vectors
    createIndexVector(kpts1p, k1p, u_bin_num, v_bin_num);
    createIndexVector(kpts2p, k2p, u_bin_num, v_bin_num);
    createIndexVector(kpts1c, k1c, u_bin_num, v_bin_num);
    createIndexVector(kpts2c, k2c, u_bin_num, v_bin_num);
    
    // for all points do
    for (i1p=0; i1p<kpts1p.size(); i1p++) {
      ...
      // compute row and column of statistics bin to which this observation belongs
      int32_t u_bin = min((int32_t)floor(u1p/(float)param.match_binsize),u_bin_num-1);
      int32_t v_bin = min((int32_t)floor(v1p/(float)param.match_binsize),v_bin_num-1);
      int32_t stat_bin = v_bin*u_bin_num+u_bin;

      // match in circle
      findMatch(kpts1p,descriptors_p_1,i1p,kpts2p,descriptors_p_2,k2p,u_bin_num,v_bin_num,stat_bin,i2p,0,false,use_prior);
      ...
      if (Tr_delta) {
        ...
        double d = max((double)(u1p-u2p),1.0);
        double x1p = (u1p-param.cu)*param.base/d;
        double y1p = (v1p-param.cv)*param.base/d;
        ...
        findMatch(kpts2p,descriptors_p_2,i2p,kpts2c,descriptors_c_2,k2c,u_bin_num,v_bin_num,stat_bin,i2c,1,true,use_prior,u2c_,v2c_);
      } else {
        findMatch(kpts2p,descriptors_p_2,i2p,kpts2c,descriptors_c_2,k2c,u_bin_num,v_bin_num,stat_bin,i2c,1,true,use_prior);
      }
      findMatch(kpts2c,descriptors_c_2,i2c,kpts1c,descriptors_c_1,k1c,u_bin_num,v_bin_num,stat_bin,i1c,2,false,use_prior);
      if (Tr_delta)
        findMatch(kpts1c,descriptors_c_1,i1c,kpts1p,descriptors_p_1,k1p,u_bin_num,v_bin_num,stat_bin,i1p2,3,true,use_prior,u1p,v1p);
      else
        findMatch(kpts1c,descriptors_c_1,i1c,kpts1p,descriptors_p_1,k1p,u_bin_num,v_bin_num,stat_bin,i1p2,3,true,use_prior);
...
void Matcher_SIFT::matching (int32_t *m1p,int32_t *m2p,int32_t *m1c,int32_t *m2c,
...
void Matcher_SIFT::removeOutliers (vector<Matcher_SIFT::p_match> &p_matched,int32_t method) {
  ...
  vector<Matcher_SIFT::p_match> p_matched_copy;  
...
bool Matcher_SIFT::parabolicFitting(const uint8_t* I1_du,const uint8_t* I1_dv,const int32_t* dims1,
...
void Matcher_SIFT::relocateMinimum(const uint8_t* I1_du,const uint8_t* I1_dv,const int32_t* dims1,
                              const uint8_t* I2_du,const uint8_t* I2_dv,const int32_t* dims2,
                              const float &u1,const float &v1,
                              float       &u2,float       &v2,
                              uint8_t* desc_buffer) {
...
void Matcher_SIFT::refinement (vector<Matcher_SIFT::p_match> &p_matched,int32_t method) {
  ...
  vector<Matcher_SIFT::p_match> p_matched_copy = p_matched;
  p_matched.clear();
  ...
  // for all matches do
  for (vector<Matcher_SIFT::p_match>::iterator it=p_matched_copy.begin(); it!=p_matched_copy.end(); it++) {
...
float Matcher_SIFT::mean(const uint8_t* I,const int32_t &bpl,const int32_t &u_min,const int32_t &u_max,const int32_t &v_min,const int32_t &v_max) {
  float mean = 0;
