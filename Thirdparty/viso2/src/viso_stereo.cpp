...
bool VisualOdometryStereo::process (const cv::Mat& I1, const cv::Mat& I2, bool reset, bool replace) {
  // matcher->pushBack(I1, I2, replace);
  // if (Tr_valid) matcher->matchFeaturesSIFT(2, reset, &Tr_delta);
  // else          matcher->matchFeaturesSIFT(2, reset);
#ifdef PROFILING
  std::chrono::steady_clock::time_point t1 = std::chrono::steady_clock::now();
#endif
  matcher->pushBackCUDA(I1, I2, replace);
#ifdef PROFILING
  std::chrono::steady_clock::time_point t2 = std::chrono::steady_clock::now();
#endif
  if (Tr_valid) matcher->matchFeaturesCUDA(2, reset, &Tr_delta);
  else          matcher->matchFeaturesCUDA(2, reset);
#ifdef PROFILING
  ...
  cout << "Odometer Profiling (CudaSIFT): " << std::chrono::duration_cast<std::chrono::duration<double> >(t2 - t1).count() << endl;
  cout << "Odometer Profiling (Match Features): " << std::chrono::duration_cast<std::chrono::duration<double> >(t3 - t2).count() << endl;
  cout << "Odometer Profiling (Bucket Features): " << std::chrono::duration_cast<std::chrono::duration<double> >(t4 - t3).count() << endl;
  cout << "Odometer Profiling (Get Matches): " << std::chrono::duration_cast<std::chrono::duration<double> >(t5 - t4).count() << endl;
  cout << "Odometer Profiling (Update Motion): " << std::chrono::duration_cast<std::chrono::duration<double> >(t6 - t5).count() << endl;
#endif
  return success;
}

vector<double> VisualOdometryStereo::estimateMotion (vector<Matcher_SIFT::p_match> p_matched) {
...
void VisualOdometryStereo::copyMatchData(std::vector<cv::KeyPoint> &kpts_l, std::vector<cv::KeyPoint> &kpts_r, cv::Mat &desc_l, cv::Mat &desc_r,
  std::vector<Matcher_SIFT::p_match> &matches_circle, std::vector<cv::DMatch> &matches_lr) {
  matcher->copyMatchData(kpts_l, kpts_r, desc_l, desc_r, matches_circle, matches_lr);
}

vector<int32_t> VisualOdometryStereo::getInlier(vector<Matcher_SIFT::p_match> &p_matched,vector<double> &tr) {
...
VisualOdometryStereo::result VisualOdometryStereo::updateParameters(vector<Matcher_SIFT::p_match> &p_matched,vector<int32_t> &active,vector<double> &tr,double step_size,double eps) {
...
void VisualOdometryStereo::computeObservations(vector<Matcher_SIFT::p_match> &p_matched,vector<int32_t> &active) {
