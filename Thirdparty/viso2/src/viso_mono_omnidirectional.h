...
public:
  ...
  bool process (const cv::Mat& I1, bool reset = true, bool replace = false);
...
private:
  ...
  std::vector<double>  estimateMotion (std::vector<Matcher_SIFT::p_match> p_matched);  
  Matrix               smallerThanMedian (Matrix &X,double &median);
  void                 fundamentalMatrix (const std::vector<Matcher_SIFT::p_match_3d> &p_matched_3d,const std::vector<int32_t> &active,Matrix &F);
  void                 EtoRt(Matrix &E,std::vector<Matcher_SIFT::p_match_3d> &p_matched_3d,Matrix &X,Matrix &R,Matrix &t);
  int32_t              triangulateChieral (std::vector<Matcher_SIFT::p_match_3d> &p_matched_3d,Matrix &R,Matrix &t,Matrix &X);
  std::vector<int32_t> getInlier (std::vector<Matcher_SIFT::p_match_3d> &p_matched_3d,Matrix &F);
  ...
  Matcher_SIFT::p_match_3d  projectIntoUnitSphere(Matcher_SIFT::p_match);
  Matcher_SIFT::p_match     projectIntoImage(Matrix X, Matrix P1, Matrix P2);
  std::vector<Matcher_SIFT::p_match_3d> projectMatches(std::vector<Matcher_SIFT::p_match> matches);
  std::vector<Matcher_SIFT::p_match> reprojectMatches(Matrix X, Matrix P1, Matrix P2);
