...
private:
  ...
  bool                 normalizeFeaturePoints (std::vector<Matcher_SIFT::p_match> &p_matched,Matrix &Tp,Matrix &Tc);
  void                 fundamentalMatrix (const std::vector<Matcher_SIFT::p_match> &p_matched,const std::vector<int32_t> &active,Matrix &F);
  void                 EtoRt(Matrix &E,Matrix &K,std::vector<Matcher_SIFT::p_match> &p_matched,Matrix &X,Matrix &R,Matrix &t);
  int32_t              triangulateChieral (std::vector<Matcher_SIFT::p_match> &p_matched,Matrix &K,Matrix &R,Matrix &t,Matrix &X);
  std::vector<int32_t> getInlier (std::vector<Matcher_SIFT::p_match> &p_matched,Matrix &F);
