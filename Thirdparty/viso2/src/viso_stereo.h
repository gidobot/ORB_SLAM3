...
public:
  ...
  bool process (const cv::Mat& I1, const cv::Mat& I2, bool reset=true, bool replace=false);
...
private:
  ...
  result               updateParameters(std::vector<Matcher_SIFT::p_match> &p_matched,std::vector<int32_t> &active,std::vector<double> &tr,double step_size,double eps);
  void                 computeObservations(std::vector<Matcher_SIFT::p_match> &p_matched,std::vector<int32_t> &active);
  void                 computeResidualsAndJacobian(std::vector<double> &tr,std::vector<int32_t> &active);
  std::vector<int32_t> getInlier(std::vector<Matcher_SIFT::p_match> &p_matched,std::vector<double> &tr);
