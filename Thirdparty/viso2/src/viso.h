...
public:
  ...
  // general parameters
  struct parameters {
    Matcher_SIFT::parameters         match;            			  // matching parameters
  ...
  bool process (std::vector<Matcher_SIFT::p_match> p_matched_) {
  ...
  std::vector<Matcher_SIFT::p_match> getMatches () { return matcher->getMatches(); }
...
protected:
  ...
  virtual std::vector<double> estimateMotion (std::vector<Matcher_SIFT::p_match> p_matched) = 0;
